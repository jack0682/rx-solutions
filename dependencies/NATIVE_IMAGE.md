...
The [DHI PTY investigation](../native/dhi/README.md) builds three immutable source archives and exact controller-manager package4.48.0. Upstream floating `.repos` entries are not used. The image build rechecks the prepared source inventory before compiling. Direct component tests exercise the simulated ownership mechanism, while registered release admission remains authority-gated because its signed resident path is not qualified. Development signing custody does not change that classification. This is not physical support or general Compose/s6 integration.
