...
`solutions-startup.json`은 관리 모드와 영속 기동/종료를 시험하는 모의 계획이다. 자사 profile ID를 검증하지만 시작하는 것은 release-owned 상태 HTTP 서비스 하나뿐이다. 장비·controller 준비나 qualification을 만들지 않는다.
