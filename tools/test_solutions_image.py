...
try:
    ...
    assert status['native_packages']==42 and status['support_profiles']==22
    ...
    with tempfile.TemporaryDirectory(prefix='rx-policy-tamper-') as temporary:
        replacement=Path(temporary)/'policy.onnx';replacement.write_bytes(b'not-a-qualified-policy')
        replacement.chmod(0o644)
        denied=subprocess.run(['docker','run','--rm','--network','none','--read-only','--cap-drop','ALL','--security-opt','no-new-privileges','-v',str(replacement)+':/opt/rx/robotis/share/ai_sapiens_sim2real/assets/k1/locomotion/velocity/walk_default/exported/policy.onnx:ro',a.image,'inspect'],capture_output=True,text=True)
        assert denied.returncode!=0 and 'policy asset integrity differs' in denied.stderr
    image=json.loads(run('docker','image','inspect',a.image))[0]
    result={'schema':'rx.solutions-image-smoke.v1','status':'PASS','image_id':image['Id'],'os':image['Os'],'architecture':image['Architecture'],'user':inspected['Config']['User'],'read_only_root':True,'cap_drop':['ALL'],'devices':[],'startup':status,'process_list':process_list,'sigterm_exit':after['State']['ExitCode'],'tampered_policy_rejected':True,'limitations':['read-only software diagnostics only','no automatic Host/driver/BT launch','no physical qualification or performance guarantee','operator API delegation not connected']}
